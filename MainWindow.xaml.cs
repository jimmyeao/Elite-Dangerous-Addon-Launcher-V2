...

{
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        // Field to track if window is being automatically resized
        private bool isAutomaticResize = false;
        ...
        public double AppNameColumnWidth
        {
            get
            {
                return Properties.Settings.Default.AppNameColumnWidth;
            }
            set
            {
                Properties.Settings.Default.AppNameColumnWidth = value;
            ...
            // Add this to the MainWindow constructor after LoadProfilesAsync
        public MainWindow(string profileName = null)
        {
            ...
            this.MinWidth = 850;  // Increased to ensure all columns fit
            this.MinHeight = 300;

            // Apply saved window size (with minimum width check)
            ...
            // Use default size if stored values are too small
            this.Width = (storedWidth >= this.MinWidth) ? storedWidth : 850;
            this.Height = (storedHeight >= this.MinHeight) ? storedHeight : 450;

            // Position will be applied in OnContentRendered
            // We don't set Top/Left here anymore
        ...
        // Add a method to handle window SizeChanged event
        ...
        protected override void OnContentRendered(EventArgs e)
        {
            ...
            // If this is first run or position was reset
            ...
            else
            {
                ...
                // Simple check if window would be visible on screen
                bool isOnScreen = false;

                // Check if at least part of the window would be visible on the virtual screen
                if (left < SystemParameters.VirtualScreenWidth && top < SystemParameters.VirtualScreenHeight &&
                    left + this.Width > SystemParameters.VirtualScreenLeft && top + this.Height > SystemParameters.VirtualScreenTop)
                {
                    isOnScreen = true;
                }
        ...
        private void AdjustWindowSizeToContent()
        {
            ...
            Dispatcher.InvokeAsync(() =>
            {
                try
                {
                    ...
                    {
                        this.Left = Math.Max(0, SystemParameters.VirtualScreenWidth - this.Width);
                    }

                    // Force layout update
                    this.UpdateLayout();

                    Log.Information($"Window resized to height: {this.Height}, width: {this.Width} for {itemCount} items");
                }
                catch (Exception ex)
                {
        ...
        private async void MainWindow_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            // Ensure the window is properly sized after loading data
        ...
        // Add this new method to ensure correct initial sizing
        private void EnsureCorrectWindowSize()
        {
            // Force layout update to ensure controls are properly measured
            UpdateLayout();

            // Check if we have a DataGrid with items
            if (AddonDataGrid != null && AddonDataGrid.Items.Count > 0)
            {
                // Make sure columns are properly sized
                AddonDataGrid.UpdateLayout();

                // Calculate the minimum width needed for all columns
                double requiredWidth = 0;

                // Add width for each column except the App Name column (which is star-sized)
                foreach (var column in AddonDataGrid.Columns)
                {
                    if (column.Header.ToString() != "App Name")
                    {
                        // For non-App Name columns, estimate their required width
                        requiredWidth += 110; // Approximate width for button columns
                    }
                }

                // Add space for the App Name column and the logo
                requiredWidth += 300 + 150; // App name + logo width + margins
            ...
            // Also run the existing window size adjustment
            AdjustWindowSizeToContent();
        }
        private void MainWindow_SizeChanged(object sender, SizeChangedEventArgs e)
        {
            // Save window size when user manually resizes
            Properties.Settings.Default.MainWindowSize = new System.Drawing.Size((int)this.Width, (int)this.Height);
            Properties.Settings.Default.Save();
        }
